...
共识验证层：每次 PoAI 验证从全网验证者中随机抽选至少 3 个节点。使用 VRF（可验证随机函数）保证选择过程随机且可验证。攻击者无法预测哪些验证者会被选中。

异常检测模型：AI 模型识别异常行为模式（如家用车突然每天行驶 1000 公里）。异常行为触发人工复审或质押冻结。连续异常导致永久禁入。
...
轻微违规：数据异常 1 次、短时间离线、轻微质量问题。扣除 5-10% 质押金，冻结奖励，降低信誉。持续正常行为可恢复（每月 +5 分信誉）。

中度违规：重复异常 3 次、长时间离线、多次质量问题。扣 10-30%，冻结 7-14 天，降级为低贡献节点。需 DAO 投票决定恢复。

严重违规：伪造数据、恶意攻击、协同作弊。扣 30-70%，奖励全部冻结，禁挖 14-60 天。极难恢复，需社区投票。

恶意攻击：系统性攻击、双花攻击、长程攻击。扣 70-100%，永久封禁，不可逆。

验证者违规：双签、审核失职、串谋作弊。扣 30-100%，失去验证资格，永久禁入验证者集合。

### 惩罚公式

Slashing Amount = 节点质押量 × 违规级别系数 × 信用因子

违规级别系数：轻微违规 5-10%、中度违规 10-30%、严重违规 30-70%、恶意攻击 70-100%、验证者违规 30-100%。

信用因子：高信誉节点（90+ 分）系数 ×0.8（减轻惩罚）、中信誉节点（60-90 分）系数 ×1.0（标准惩罚）、低信誉节点（<60 分）系数 ×1.5（加重惩罚）。

示例：节点质押 10,000 AIXC，中度违规（系数 20%），中信誉节点（信用因子 1.0），罚没金额 = 10,000 × 20% × 1.0 = 2,000 AIXC。
...
数据与 AI 服务费：AI 模型调用费（调用 xEAI 算法模型的使用费）、数据市场手续费（数据交易平台的交易手续费，如 5-10%）。

资产服务费：设备接入费用（EAI 设备首次接入网络的注册费）、RWA 铸造费（智能资产上链时的铸造手续费，如 0.5%）、RWA 交易费（资产流转时的交易手续费，如 2%）。
